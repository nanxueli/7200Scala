...
object Fuzzy {
  ...
  def apply(i: String, x: Option[String], f: Option[String], e: Option[String]): Fuzzy = {
    ...
    def fuzzFactor(z: String, n: Int): Double = z.toInt/Math.pow(10, n-2)
    /*{
      val bd = BigDecimal(z.toInt)/BigDecimal(Math.pow(10, n-1))
      bd.doubleValue()
    }*/
    // TODO 5 points
    //Math.pow(z.toDouble/Math.pow(10, n-1),20) 
    //def fuzzFactor(z: Int, n: Double): Double = 1/(Math.sqrt(2*Math.PI)*(z.toDouble+n) )// TODO 5 points
    //val y = exp match {case Some(x) => x; case None => 1}
    //val fuzz = for { z <- f; w <- fraction} yield fuzzFactor(z,w._2)*y
    //val fuzz = for { z <- i; w <- fraction; y <- (exp orElse Some(1.0)) } yield fuzzFactor(z,w._1)*y
...
//    println(s"creating Fuzzy: with parameters: $i $x $f $e with intermediate results fraction=$fraction, fuzz=$fuzz, exponent=$exponent")
  ...
  def parse(s: String): Try[Fuzzy] = 
    parser.parseAll(parser.fuzzy,s) match {
      case parser.Success(x,_) => Success(x) // TODO 3 marks
