...
trait ProductStream[X <: Product] {
  ...
  def map[Y <: Product](f: X=>Y): ProductStream[Y] = {
    val y = for{tuple <- this.tuples} yield f(tuple)
    this.ensuring(y.isInstanceOf[Stream[Product]]).asInstanceOf[ProductStream[Y]]
  } // TODO 5
  /**
   * flatMap method
	 * @param f function to be applied to each tuple
  ...
  def flatMap[Y <: Product](f: X=>GenTraversableOnce[Y]): ProductStream[Y] = {
    val y = for{tuple <- tuples} yield f(tuple)
    this.ensuring(y.isInstanceOf[Stream[Product]]).asInstanceOf[ProductStream[Y]]
  }  // TODO 5
  /**
   * toMap method
...
abstract class ProductStreamBase[X <: Product] extends ProductStream[X] {
  ...
  def asMaps: Stream[Map[String,Any]] = {
    val stream = for{
     ...
     case (x,y) => Map(this.header.head-> x, this.header.last -> y)
    }
    stream.toStream
    } // TODO 14
}
...
object TupleStream {
  ...
  def toTuple[X <: Product](ats: Seq[Try[Any]]): Try[X] = Try(Tuples.toTuple(MonadOps.sequence(ats).get).asInstanceOf[X]) // ???TODO 8 Hint: use MonadOps.sequence; Tuples.toTuple; and asInstanceOf
...
case class CsvParser(
    ...
    text:String = """[A-Za-z0-9,|!\ \-.]+""",//"""^[a-zA-Z0-9~@#$^*()_+=[\]{}|\\,.?: -]*$+""",//"""(\w+)|([$&+,:;=?@#|'<>.-^*()%!])""",
  ...
  def row: Parser[List[String]] =(term~((delimiter~>term)*)) ^^ {case r~rs => r::rs}// TODO 3: row ::= term { delimiter term }
  def term: Parser[String] = quoteChar~>text.r<~quoteChar|(text.replace(delimiter, "").r) ^^ { case text => text}// TODO 7: term ::= quoteChar text quoteChar | text
